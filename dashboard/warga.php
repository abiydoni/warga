...
<head>
  <meta charset="UTF-8">
  <title>Data Warga - <?= htmlspecialchars($user['username']) ?></title>
  ...
  <style>
    ...
    .bg-hero {
      ...
      /* Tambahkan overlay putih transparan agar lebih terang */
      position: relative;
    }
    .bg-hero::before {
      content: '';
      ...
      background: rgba(255,255,255,0.8); /* overlay putih terang */
      z-index: 0;
    }
    .main-content {
      position: relative;
...
<body class="bg-hero min-h-screen text-black font-sans antialiased">
  <div class="main-content min-h-screen p-6">
    ...
    <div class="table-data">
        <div class="order">
            <!-- Search dan Reset + Action Buttons -->
    ...
    <div id="modalBiodata" class="modal-overlay hidden">
        <div class="modal-container bg-white text-black rounded-lg shadow-xl p-4 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div id="biodataContent" class="text-black">
    ...
    <div id="modalKK" class="modal-overlay hidden">
        <div class="modal-container bg-white text-black rounded-lg shadow-xl p-4 w-full max-w-6xl max-h-[90vh] overflow-y-auto">
            <div id="kkContent" class="text-black">
                <!-- Content akan diisi oleh JavaScript -->
            </div>
        </div>
    </div>
    ...
    <div id="loadingModal" class="modal-overlay hidden">
        <div class="modal-container bg-white text-black rounded-lg shadow-xl p-6 w-full max-w-md">
    ...
    <div id="modal" class="modal-overlay hidden">
        <div class="modal-container bg-white text-black rounded-lg shadow-xl p-4 w-full max-w-xs max-h-[90vh] overflow-y-auto">
            ...
            <form id="wargaForm" class="text-sm text-black">
                ...
                <div class="form-3col">
                    ...
                    <div class="flex flex-col items-center justify-start mt-2">
                        <label class="foto-upload-label">
                            <img src="assets/images/users.gif" id="fotoPreview" class="foto-preview" alt="Foto Profil">
    ...
    function loadData() {
      $.post('api/warga_action', { action: 'read' }, function(data) {
    ...
    function showBiodata(nik) {
      ...
      $.post('api/warga_action', { action: 'get_warga_by_nik', nik: nik }, function(data) {
        // Data dari API sudah object, tidak perlu JSON.parse
        displayBiodata(data);
      }).fail(function(xhr, status, error) {
    ...
    function showKK(nikk) {
      ...
      $.post('api/warga_action', { action: 'get_kk_by_nikk', nikk: nikk }, function(data) {
        // Data dari API sudah object, tidak perlu JSON.parse
        displayKK(data);
      }).fail(function(xhr, status, error) {
    ...
    function loadProvinsi() {
      return new Promise((resolve, reject) => {
        ...
        $.get('api/wilayah', { action: 'provinsi' }, function(data) {
    ...
    function loadKota(provinsi_id) {
      return new Promise((resolve, reject) => {
        ...
        $.get('api/wilayah', { action: 'kota', provinsi_id: provinsi_id }, function(data) {
    ...
    function loadKecamatan(kota_id) {
      return new Promise((resolve, reject) => {
        ...
        $.get('api/wilayah', { action: 'kecamatan', kota_id: kota_id }, function(data) {
    ...
    function loadKelurahan(kecamatan_id) {
      return new Promise((resolve, reject) => {
        ...
        $.get('api/wilayah', { action: 'kelurahan', kecamatan_id: kecamatan_id }, function(data) {
    ...
    $(document).ready(function() {
        ...
        $('#foto_file').on('change', function() {
          ...
          if (file) {
            ...
            if (!allowedTypes.includes(file.type)) {
              ...
              this.value = '';
              $('#fotoPreview').attr('src', 'assets/images/users.gif');
              return;
            }
            ...
            if (file.size < 10 * 1024) {
              showToast('Ukuran file terlalu kecil. Minimal 10KB', 'error');
              this.value = '';
              $('#fotoPreview').attr('src', 'assets/images/users.gif');
              return;
            }
            ...
            img.onload = function() {
              ...
              if (width > 1920 || height > 1080) {
                showToast('Dimensi gambar terlalu besar. Maksimal 1920x1080 pixel', 'error');
                $('#foto_file')[0].value = '';
                $('#fotoPreview').attr('src', 'assets/images/users.gif');
                return;
              }
              ...
              if (width < 100 || height < 100) {
                showToast('Dimensi gambar terlalu kecil. Minimal 100x100 pixel', 'error');
                $('#foto_file')[0].value = '';
                $('#fotoPreview').attr('src', 'assets/images/users.gif');
                return;
              }
      ...
      $(document).on('click', '#exportBtn', function() {
        ...
        $.post('api/warga_action', { action: 'read' }, function(data) {
          ...
          try {
            const warga = Array.isArray(data) ? data : [];
            if (!warga.length) {
            ...
            setTimeout(() => {
              $('#loadingModal').removeClass('modal-show').addClass('hidden');
              showToast('Export data warga ke Excel berhasil!', 'success');
            }, 1000);
            
          } catch (e) {
            $('#loadingModal').removeClass('modal-show').addClass('hidden');
            showToast('Error saat export: ' + e.message, 'error');
          }
        }).fail(function(xhr, status, error) {
          $('#loadingModal').removeClass('modal-show').addClass('hidden');
              ...
              setTimeout(() => {
                ...
                try {
                  ...
                  Swal.fire({
                    icon: 'error',
                    title: 'Import Data',
                    html: (typeof response === 'object' ? JSON.stringify(response) : response).replace(/\n/g, '<br>'),
                    confirmButtonText: 'OK',
                    allowOutsideClick: false
                  });
                }
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            if (result.success) {
              ...
              Swal.fire({
                title: 'Gagal',
                text: result.message || 'Gagal simpan data warga',
                icon: 'error',
                confirmButtonText: 'OK'
              });
            }
        ...
        try {
          ...
          if (data.foto) {
            ...
            if (data.foto && data.foto !== '') {
              ...
              if (!/^assets\//.test(fotoSrc) && !/^http/.test(fotoSrc)) {
                fotoSrc = 'assets/images/warga/' + fotoSrc;
              }
          ...
          showToast('Error parsing data: ' + e.message, 'error');
        }
      });
      ...
      $(document).on('click', '.deleteBtn', function() {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.post('api/warga_action', { action: 'delete', id_warga: id }, function(res) {
              // Handle response that's already parsed as object by jQuery
              let result;
              if (typeof res === 'object') {
                result = res;
